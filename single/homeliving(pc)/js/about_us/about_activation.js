    ...
    // //元素点击事件
     ...
     function callback(e){
          ...
          if(target.className ===''&&target.tagName==='A'){
             for(var i =1,len=liAs.length-1;i<len;i++ ){="" lias[i].classname="" ;="" }="" target.classname="bottom_bar_color" index="parseInt(target.textContent);" }else="" if(target.classname="=='bottom_bar_color'){" return="" false;="" 前一个页面点击before="" if(target="==before){" lias[index].classname="" index--;="" if(index<1){="" 下一个页面after="" index++;="" if(index="">6){
               index=6
             }
